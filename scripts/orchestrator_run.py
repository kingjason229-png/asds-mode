...
def normalize_task(task: dict) -> dict:
    ...
    task.setdefault("affected_files", [])
    task.setdefault("verification_summary", "")
    task.setdefault("verification_artifacts", [])
    task.setdefault("blocker_type", None)
...
def finalize_after_agents(task: dict) -> str:
    ...
    if status == "RETRY":
        ...
        task["status"] = "PENDING"
        atomic_write_tasks(reloaded)
        append_progress(f"task {task['id']} returned for retry", validation="qa requested retry", artifacts=str(TASKS_FILE), next_step="stop")
    ...
    if status in {"DONE", "BLOCKED"}:
        ...
        atomic_write_tasks(reloaded)
        append_progress(f"task {task['id']} closed as {status}", validation="qa final decision", artifacts=str(TASKS_FILE), next_step="stop")
